...
### Abstract
**PRECISE** (*Prediction of REsponse using Cell-type Inference and Single-cell Embedding*) integrates single-cell foundation model embeddings (**scGPT**) with language model embeddings (**Mixedbread**) into a combined representation (**scMIXE**) to predict anti–PD-1 treatment response. Applied to breast cancer cohorts (Bassez and I-SPY2), scMIXE improved clustering quality, recovered key immune populations, and achieved the highest predictive accuracy (AUC ≈ 0.83), outperforming PD-L1 and clinical baselines. PRECISE provides an interpretable, cell type-specific framework for biomarker discovery in immunotherapy.

...
embeddings <- read_feather("bassez_scgpt_mxbai_concat_768.feather")

# Convert embeddings to a matrix format 
...
seurat_obj <- FindClusters(seurat_obj, resolution = 0.040)
...
DimPlot(
  ...
  label = TRUE,
  raster = FALSE
) + ggtitle("UMAP of Clustering Results (Resolution = 0.040)") + 
  theme(
...
write.csv(markers, "bassez_concat_06-28_15clusters_all_markers.csv", row.names = FALSE)


## ---- Create Pseudo-Bulk Matrix and Annotate with SingleR ----
all_markers <- read.csv("bassez_concat_06-28_15clusters_all_markers.csv")

# Load reference dataset
ref <- celldex::HumanPrimaryCellAtlasData()

# Create pseudo-bulk matrix of cluster marker genes for cell type annotation using SingleR
...
write.csv(cluster_annotations, "bassez_concat_06-28_15clusters_all_markers_pred_celltypes.csv", row.names = FALSE)
...
DimPlot(
  ...
  cols = custom_colors,
  raster = FALSE
) + ggtitle("UMAP of Predicted Cell Types from 15 Clusters") +
  theme(
    plot.title = element_text(size = 14, hjust = 0.5),
    legend.text = element_text(size = 12),
    axis.text = element_text(size = 12),
